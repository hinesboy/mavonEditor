...
export const insertCodeBlock = ($vm) => {
  ...
  if (isExistSelectionObj(obj)) {
    ...
    const { selectionStart: startPos, selectionEnd: endPos, value: tmpStr } = obj
    ...
    if (startPos === endPos) {
      ...
      if (isCancelCodeBlock(obj)) {
        ...
        // 确定
        const value = tmpStr.substring(startPos, endPos)
...
function insertCodeBlockToVM(language, content, selectionObj) {
  ...
  const { selectionStart: startPos, selectionEnd: endPos, value: tmpStr } = selectionObj
...
function removeCodeBlockFromVM(selectionObj) {
  ...
  const { defaultLanguageText: language } = code_structure
  const { selectionStart: startPos, selectionEnd: endPos, value: content } = selectionObj
  const selectedValue = content.substring(startPos, endPos)
